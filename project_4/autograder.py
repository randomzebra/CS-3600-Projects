...
def sameList(list1,list2,floatDif):
    ...
    for index in range(len(list1)):
        if isinstance(list1[index],float):
            ...
            if not sameList(list1[index],list2[index],floatDif):
                return False
...
def copyWeights(nnet1, nnet2):
    for l in range(len(nnet1.layers)):
        for n in range(len(nnet1.layers[l])):
...
def q2Test(testFile):
    if 'sigmoidDeriv' in testFile.name:
        ...
        if 'update' in testFile.name:
            solution = []
            for example in examples:
                oldWeights = sPercep.weights[:]
                solution.append(sPercep.updateWeights(example[0],0.1,0.67))
                if sameList(oldWeights, sPercep.weights, 0.01):
                    raise Exception("updateWeights should actually change the self.weights list")
...
def runTests(q,points=2):
    ...
    for fileName in [name for name in listdir('test_cases/'+q) if 'test' in name]:
        ...
        try:
            ...
            correct= False
            ldic=locals()
            exec(solution[0].strip(),globals(), ldic)
            correct=ldic["correct"]
        except Exception as e:
        ...
        print('--------------------------------------------------------------------')
    if total==possible:
        print('Total: %d/%d'%(points,points))
        print('All tests passed')
        print('Done\n____________________________________________________________________\n')
        return (points,points)
    else:
        print('Total: 0/%d'%points)
...
def makePickledObjects():
    ...
    print(net)
    f = open('test_cases/nnet','wb')
    pickle.dump(net,f)
    f.close()
    
    f = open('test_cases/percep','wb')
    pickle.dump(net.layers[0][1],f)
    f.close()
